  ...
  https.get(targetUrl, { headers: { 'User-Agent': 'Mozilla/5.0 DPE-App/1.0' } }, (apiRes) => {
    ...
    apiRes.on('end', () => {
      res.writeHead(200, {
        ...
        'Access-Control-Allow-Origin': '*'const http = require('http');
const https = require('https');
const url = require('url');
...
function fetchUrl(targetUrl) {
  return new Promise((resolve, reject) => {
    const options = {
      headers: {
        'User-Agent': 'Mozilla/5.0 (compatible; DPE-App/1.0)',
...
const server = http.createServer(async (req, res) => {
  ...
  // Diagnostic — voir les vraies colonnes disponibles
  if (path === '/debug') {
    try {
      const r = await fetchUrl(
        'https://data.ademe.fr/data-fair/api/v1/datasets/dpe03existant/lines?q=Gardanne&q_fields=adresse_ban&page=1&size=1'
      );
      sendJSON(res, r.body);
    } catch (e) { sendError(res, e.message); }
    return;
  ...
  // Recherche DPE avec les nouveaux noms de colonnes (2025)
  if (path === '/dpe') {
    try {
      // Nouveaux noms de colonnes depuis la refonte février 2025
      const fields = [
        'numero_dpe',
        'date_etablissement_dpe',
        'etiquette_dpe',
        'etiquette_ges',
        'conso_5_usages_e_finale',        // nouveau nom pour consommation_energie
        'emission_ges_5_usages',           // nouveau nom pour emission_ges
      ...
      const r = await fetchUrl(
        'https://data.ademe.fr/data-fair/api/v1/datasets/dpe03existant/lines?q=' + encodeURIComponent(q) +
        '&q_fields=adresse_ban&page=1&size=8&sort=-date_etablissement_dpe&select=' + fields
      );

      if (r.status === 200) {
        // Normaliser les noms vers l'ancien format pour le frontend
        try {
          const data = JSON.parse(r.body);
          if (data.results) {
            data.results = data.results.map(row => ({
              ...row,
              consommation_energie: row.conso_5_usages_e_finale || row.consommation_energie,
              emission_ges: row.emission_ges_5_usages || row.emission_ges,
            }));
          }
          sendJSON(res, data);
        } catch(e) {
        ...
        // Essai sans select pour voir l'erreur exacte
        const r2 = await fetchUrl(
          'https://data.ademe.fr/data-fair/api/v1/datasets/dpe03existant/lines?q=' + encodeURIComponent(q) +
          '&q_fields=adresse_ban&page=1&size=1'
        );
        sendJSON(res, JSON.stringify({ results: [], _debug: r2.body.substring(0, 500) }));
      }
    } catch (e) { sendError(res, e.message); }
    return;
  }
...
<head>
  ...
  <style>
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    #search-btn{padding:.75rem 1.4rem;background:#4361ee;color:white;border:none;border-radius:10px;font-size:.95rem;font-weight:600;cursor:pointer;white-space:nowrap}
...
<body>
<div class="container">
  <header><h1>🏠 Recherche DPE par adresse</h1><p>Données officielles ADEME — chauffage, énergie, isolation, classe énergétique</p></header>
  ...
  <div class="no-dpe" id="no-dpe"><p>❌ Aucun DPE trouvé pour cette adresse dans le registre ADEME.<br>Le logement n'a peut-être pas de DPE depuis juillet 2021.</p></div>
  <div id="result-area">
    ...
    <p class="section-title">⚡ Performance énergétique</p><div class="cards-grid" id="perf-grid"></div>
    <p class="section-title">🔧 Équipements</p><div class="equip-grid" id="equip-grid"></div>
    <p class="section-title">🏗️ Caractéristiques</p><div class="cards-grid" id="logement-grid"></div>
    <div class="source-note">Données ADEME · Licence Ouverte Etalab · DPE après juillet 2021</div>
  </div>
</div>
<script>
  ...
  async function doSearch(){const addr=inp.value.trim();if(!addr)return;sug.style.display='none';hideAll();document.getElementById('search-btn').disabled=true;setS('Interrogation du registre ADEME...','loading');try{const resp=await fetch('/dpe?q='+encodeURIComponent(addr));const data=await resp.json();setS('','');document.getElementById('search-btn').disabled=false;if(data.error)throw new Error(data.error);if(!data.results?.length){document.getElementById('no-dpe').style.display='block';return;}data.results.length===1?render(data.results[0]):showMulti(data.results);}catch(err){setS('Erreur : '+err.message,'error');document.getElementById('search-btn').disabled=false;}}
